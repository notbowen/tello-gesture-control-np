...
def main():
    ...
    while True:
        ...
        cv.putText(debug_image, "Battery: {}, Controller: {}".format(battery_status, "Keyboard" if KEYBOARD_CONTROL else "Gestures"), (5, 720 - 5),
                   cv.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2)
        cv.imshow('Tello Gesture Recognition', debug_image)

        # Land if battery is lower than 10
        if int(battery_status) <= 10 and int(battery_status) != -1:
            tello.land()
            tello.end()
